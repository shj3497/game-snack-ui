...
const getGameScore = async (
  ...
  try {
    const res = await queryClient.fetchQuery({
      queryKey: getCheckDrawEligibilityQueryKey(pid, {adid, idfa, userId}),
      queryFn: () => checkDrawEligibility(pid, {adid, idfa, userId}),
    ...
    const DIAGRAM = (
      res.system.shape || 'square'
    ).toLocaleLowerCase() as DiagramType;
          ...
          type:
            item.percent === undefined
              ...
              : item.percent >= 65
                ? 'success'
                : 'fail',
          score: item.percent === undefined ? undefined : `${item.percent}%`,
        };
      });
      draft.TIME = TIME;
